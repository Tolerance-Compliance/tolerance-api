...
use crate::cmmc::handler::{
    get_documents, get_far_element, get_far_element_relationships, get_far_elements,
    get_far_families, get_far_family, get_far_relationships, get_far_requirements, get_far_summary,
    get_nist_element, get_nist_element_relationships, get_nist_elements, get_nist_families,
    get_nist_family, get_nist_relationships, get_nist_requirements, get_nist_security_requirements,
    get_nist_summary, get_non_eligible_requirements, validate_poam_batch,
    validate_poam_requirement,
};
use crate::constant::{
    FAR_ELEMENT_ENDPOINT, FAR_ELEMENT_RELATIONS_ENDPOINT, FAR_ELEMENTS_ENDPOINT,
    FAR_FAMILIES_ENDPOINT, FAR_FAMILY_ENDPOINT, FAR_RELATIONSHIPS_ENDPOINT,
    FAR_REQUIREMENTS_ENDPOINT, FAR_SUMMARY_ENDPOINT, HEALTH_ENDPOINT, MCP_ENDPOINT,
    ...
    NIST_SUMMARY_ENDPOINT, POAM_NON_ELIGIBLE_REQS_ENDPOINT, POAM_VALIDATE_BATCH_ENDPOINT,
    POAM_VALIDATE_REQ_ENDPOINT,
};
...
pub fn app(state: CmmcState) -> Router {
    Router::new()
        ...
        .route(
            NIST_SECURITY_REQS_ENDPOINT,
            get(get_nist_security_requirements),
        )
        .route(NIST_RELATIONSHIPS_ENDPOINT, get(get_nist_relationships))
        .route(
            NIST_ELEMENT_RELATIONS_ENDPOINT,
            get(get_nist_element_relationships),
        )
        ...
        .route(
            POAM_NON_ELIGIBLE_REQS_ENDPOINT,
            get(get_non_eligible_requirements),
        )
        // FAR API
        ...
        .route(
            FAR_ELEMENT_RELATIONS_ENDPOINT,
            get(get_far_element_relationships),
        )
        // Middleware
        .with_state(state)
